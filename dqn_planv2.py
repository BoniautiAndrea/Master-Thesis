...
def select_action(state, init, episode):
    ...
    if sample > epsilon:
        ...
        #if steps_done < 50 and init == False:
        #    return torch.tensor([[pp.action_from_combined(state.detach().numpy()[0], (plan_x, plan_y, plan_t))]], device='cpu', dtype=torch.long)
        #else:
...
if __name__ == '__main__':
    ...
    if init:
        ...
        train_plan(policy, domain, 100, 128)
        print('Pretraining done..')
        torch.save(policy_net.state_dict(), f'{PATH}init_100_test.pt')
        #sys.exit()
    ...
    for i_episode in training_loop:
        ...
        if len(episode_rewards) >= 100 and np.mean(episode_rewards[-100:]) > TARGET_R:
            ...
            torch.save(policy_net.state_dict(), f'{PATH}dqn_planv2_it{i_episode}.pt')
            np.savetxt(f'{PATH}/dqn_planv2_it{i_episode}_rewards.txt', episode_rewards)
            break
    print('Done..')

    #torch.save(policy_net.state_dict(), f'{PATH}plan_v2.pt')
    #np.savetxt(f'{PATH}/eps40/dqn_plan_v2.txt', episode_rewards)
