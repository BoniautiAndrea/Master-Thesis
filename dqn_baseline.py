...
if __name__ == '__main__':
    ...
    for i_episode in training_loop:
        ...
        if len(episode_rewards) >= 100 and np.mean(episode_rewards[-100:]) > TARGET_R:
            ...
            torch.save(policy_net.state_dict(), f'{PATH}baseline_it{i_episode}.pt')
            np.savetxt(f'{PATH}/baseline_it{i_episode}_rewards.txt', episode_rewards)
            break

    #torch.save(policy_net.state_dict(), f'{PATH}naive400x80.pt')
    #np.savetxt(f'{PATH}/eps40/dqn_naive.txt', episode_rewards)
